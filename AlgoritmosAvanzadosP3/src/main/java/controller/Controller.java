...
public class Controller {
    ...
    public void start() {
        ...
        switch (metodo) {
            case FUERZA_BRUTA -> encontrarParejasBF(puntos);               
            case DIVIDE_Y_VENCERAS -> encontrarParejasDC(puntos, minimizar);
    ...
    private double encontrarParejasDC(Punto[] puntos, int left, int right, boolean minimizar) {
        if (right - left == 2) {
            ...
            if (minimizar) return Double.MAX_VALUE;
            ...
            if (minimizar) return Math.min(Math.min(d1, d2), d3);
            else return Math.max(Math.max(d1, d2), d3);
        }
        else {
